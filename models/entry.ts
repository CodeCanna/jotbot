...
export function insertEntry(entry: Entry) {
  try {
    ...
    console.log(`Failed to insert Journal Entry: ${err}`);
  }
}
...
export function getAllEntries(userId: number): Record<string, SQLInputValue>[] {
  ...
  try {
    const db = new DatabaseSync("db/jotbot.db");
    const queryResults = db.prepare(`SELECT * FROM entry_db WHERE userId = '${userId}' ORDER BY timestamp DESC`).all();
    for (const entry in queryResults) {
      entries.push(queryResults[entry]);
    }
    db.close();
  } catch (err) {
    console.log(`Jotbot Error: Failed retrieving all entries for user ${userId}: ${err}`);
  }
  console.log(entries);
  return entries;
}
